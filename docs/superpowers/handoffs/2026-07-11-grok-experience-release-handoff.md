...
2. **Grok B — Experience Tasks 7–8.** Start after Forms Tasks 1–6 are complete. `npm run check:transcript-review` may still report the explicit authorized-human pending state: in that state the homepage must use the tested direct-YouTube fallback with no iframe. Do not weaken or populate transcript evidence. Complete Experience Tasks 7–8 and stop; player activation and every release phase remain blocked until the human gate exits 0.

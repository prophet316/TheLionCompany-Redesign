...
- `npm run check:transcript-review` must exit 0 before any homepage featured-player composition, release-candidate audit, staged Production creation, or promotion. A passing unit suite that only proves the direct-link fallback never substitutes for the authorized-human complete listen-through evidence.

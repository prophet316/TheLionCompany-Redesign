...
- `npm run check:transcript-review` must exit 0 before the homepage teaching may become embeddable, any release-candidate audit, staged Production creation, or promotion. Homepage composition may proceed with the tested direct-YouTube fallback and no iframe. A passing unit suite that only proves that fallback never substitutes for the authorized-human complete listen-through evidence.
